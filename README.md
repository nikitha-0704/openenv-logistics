...
📄 **[Problem statement & design](./PROBLEM_STATEMENT.md)** · 📓 **[Training notebook](./notebooks/train_driver_trl.ipynb)** · 🛰️ **[Live HF Space](https://huggingface.co/spaces/nikitha04/openenv-logistics)** · 🎬 **[Push-button demo](./scripts/demo.sh)**
...
*Mean **terminal `/grader` score** per task across 3 seeds, with SEM error bars. Scripted = hand-coded `test_local.py` optimum (no LLM, upper-bound baseline); single-agent LLM = `inference.py`; multi-agent LLM = `inference_multi.py` (Dispatcher + per-truck Drivers + message bus + re-plan). **Headline: on the Hard task — VIP delivery + adversarial port strike + budget squeeze + partial obs — the multi-agent system reaches `0.467` while the single-agent LLM completely fails at the grader floor (`0.001`). Hierarchy + inter-agent messaging recovers ~47% of the scripted optimum on the hardest task purely via prompt-time architecture, no fine-tuning.** Easy floors for both LLMs (route-topology mistakes); Medium ties because the env's analytical floor for partial-information policies is `0.666`.*

![SFT loss curve from notebooks/train_driver_trl.ipynb](docs/plots/sft_loss_curve.png)
*Cross-entropy SFT loss vs. optimizer step on filtered teacher trajectories (Qwen2.5-0.5B-Instruct + LoRA r=8, T4 in Colab / HF Jobs). Training pipeline converges; the GRPO cell in section 5 of the notebook plugs `GET /grader` in directly as the reward function for the deck-recommended "training loop talks to env, not a static dataset" pattern.*
